...
export const GET: RequestHandler = async ({ request, fetch, locals, cookies }) => {
    ...
    const response = await fetch(`${baseUrl}/winner/filtered?date_from=${dateFrom}&date_to=${dateTo}&branches=${encodeURIComponent(branchId)}&draw_schedules=${encodeURIComponent(drawSchedules)}`, {
        method: 'GET',
        headers: { 'Content-Type': 'application/json',
