...
export const DELETE: RequestHandler = async ({ params, fetch, locals, cookies }) => {
    ...
    try {
        const response = await fetch(`${baseUrl}/ticket/${branchId}/${ticketSerial}`, {
            method: 'DELETE',
            headers: {
                'Authorization': `Bearer ${token}`,
                'Content-Type': 'application/json',
        ...
        if (!response.ok) {
            const errorData = await response.json();
            return new Response(JSON.stringify({ error: errorData.message || 'Error al eliminar el tiquete' }), { status: response.status });
        }
        return new Response(null, { status: 204 });
    } catch (error) {
        return new Response(JSON.stringify({ error: 'Error al eliminar el tiquete' }), { status: 500 });
    }
};
